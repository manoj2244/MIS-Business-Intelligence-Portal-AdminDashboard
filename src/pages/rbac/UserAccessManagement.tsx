...
import {
  Card, Select, Button, Space, message, Tag, Row, Col,
  Descriptions, Spin, Typography, Empty, Checkbox, Tooltip, Tabs,
} from 'antd';
import { UserOutlined, DatabaseOutlined, LockOutlined, AppstoreOutlined } from '@ant-design/icons';
import { rbacApi } from '../../services/rbacApi';
import { getRegions, getClusters, getMappings } from '../../services/hierarchyApi';
import ComponentAccessTab from './ComponentAccessTab';
      ...
      {regions.length > 0 ? (
        <>
          <div className="uam-help-box">
            <Text>
              <strong>How to use:</strong> Click a region to view clusters, click a cluster to view branches.
              {defaultKeys.size > 0 && (
                <span style={{ marginLeft: 8, color: '#f59e0b' }}>
                  <LockOutlined /> <strong>Locked items</strong> are auto-granted from HRMS and cannot be removed.
                </span>
              )}
            </Text>
          </div>
            ...
            <Col xs={24} lg={8}>
              ...
              <div className="uam-column-body">
                {regions.map(region => {
                  ...
                  const selectedBranchCount = allRegionBranchKeys.filter(k => checkedKeys.includes(k) || defaultKeys.has(k)).length;
                  ...
                  return (
                    ...
                    >
                      <Tooltip title={isLocked ? 'Auto-granted from HRMS — cannot be removed' : isPartial ? `${selectedBranchCount} of ${totalBranchCount} branches assigned` : undefined}>
                        <Checkbox
                          checked={isChecked} indeterminate={isPartial} disabled={isLocked}
                      ...
                      <Space direction="vertical" size={4} style={{ width: '100%', paddingRight: 30 }}>
                        ...
                        <div>
                          ...
                          {isPartial && <Tag color="geekblue" style={{ fontSize: 11 }}>{selectedBranchCount}/{totalBranchCount} branches</Tag>}
                        </div>
                      </Space>
                    </div>
                  );
                ...
                {selectedRegion ? (
                  clusters.filter(c => c.regionCode === selectedRegion).length > 0 ? (
                    clusters.filter(c => c.regionCode === selectedRegion).map(cluster => {
                      ...
                      const selectedClusterBranchCount = allClusterBranchKeys.filter(k => checkedKeys.includes(k) || defaultKeys.has(k)).length;
                      ...
                      return (
                        ...
                        >
                          <Tooltip title={isLocked ? 'Auto-granted from HRMS — cannot be removed' : isPartial ? `${selectedClusterBranchCount} of ${clusterBranches.length} branches assigned` : undefined}>
                            <Checkbox
                              checked={isChecked} indeterminate={isPartial} disabled={isLocked}
                              onClick={e => e.stopPropagation()}
                          ...
                          <Space direction="vertical" size={4} style={{ width: '100%', paddingRight: 30 }}>
                            ...
                            <div>
                              ...
                              {isPartial && <Tag color="geekblue" style={{ fontSize: 11 }}>{selectedClusterBranchCount}/{clusterBranches.length} branches</Tag>}
                            </div>
                          </Space>
                        </div>
                      );
                  ...
                  <div className="uam-empty-col"><Text type="secondary">Select a region to view clusters</Text></div>
                )}
              </div>
            </Col>
                ...
                {(selectedCluster || selectedRegion) ? (() => {
                  ...
                  return filteredMappings.length > 0 ? filteredMappings.map(mapping => {
                    ...
                    return (
                      ...
                      >
                        <Tooltip title={isLocked ? 'Auto-granted from HRMS — cannot be removed' : undefined}>
                          <Checkbox
                            checked={isChecked} disabled={isLocked}
                  ...
                  <div className="uam-empty-col"><Text type="secondary">Select a region or cluster to view branches</Text></div>
                )}
              </div>
            </Col>
          </Row>
  ...
  return (
    <div className="ui-page" style={{ padding: 12 }}>
      <Row gutter={[12, 12]}>
        ...
        {/* Tabbed access management */}
        <Col xs={24}>
          <Card className="pro-card-gradient uam-card" size="small" bodyStyle={{ padding: 0 }}>
            <Tabs
              ...
              items={[
                {
                  key: 'data',
                  label: <Space><DatabaseOutlined />Data Access</Space>,
                  children: (
                ...
                {
                  key: 'component',
                  label: <Space><AppstoreOutlined />Component Access</Space>,
                  children: (
                    <div style={{ padding: '0 0 16px' }}>
                      <ComponentAccessTab userCode={selectedUser} />
                    </div>
                  ),
                },
              ]}
